document.addEventListener("DOMContentLoaded", function() {
    ...
    if (cartData.length > 0) {
        cartData.forEach(item => {
            ...
            productName.textContent = `Producto: ${item.nombre}`;

            const productCost = document.createElement('p');
            productCost.textContent = `Costo: $${item.costo.toFixed(2)} ${item.moneda}`;
            ...
            const calculateSubtotal = () => {
                ...
                subtotal.textContent = `Subtotal: $${(item.costo * quantity).toFixed(2)} ${item.moneda}`;
            };
            calculateSubtotal();
